...
from plugp100.models.device import LightDeviceState
...
class LightComponent(DeviceComponent):
    ...
    def color_temp_range(self) -> Tuple[int, int]:
        if self._state.color_temp_range is not None:
            return self._state.color_temp_range
        return 2500, 6500
    ...
    async def update(self, current_state: dict[str, object] | None = None):
        self._state = LightDeviceState.try_from_json(current_state or {}).get_or_raise()
